...
void test1i(void) {
    ...
    SEND_MESSAGE(td->target_pid, "this is boooollshit", td->send_length, &Z502_REG9);
...
void test1m(void) {
    ...
    int i=0;
    int n=0;
    static char HostA[4][50] = { "Card A",
        "Card B","Card C","Card D"};// Use as pointer to data
    ...
    CREATE_PROCESS("test1m_1", test1m_echo, NORMAL_PRIORITY, &Z502_REG3,
                   &Z502_REG9);
...
void test1m_echo(void) {
    TEST1m_ECHO_DATA *td;                          // Use as ptr to data
     char *wordCursor;
    int i=0;
    static char HostB[4][50] = { "Card A",
        "Card B","Card C","Card D"};
    
    td = (TEST1m_ECHO_DATA *) calloc(1, sizeof(TEST1m_ECHO_DATA));
    if (td == 0) {
        printf("Something screwed up allocating space in test1m_echo\n");
    }
...
void test1j_echo(void) {
    ...
    while (1) {       // Loop forever.
        ...
        td->target_pid = td->actual_source_pid;
        strcpy(td->msg_sent, "what the hell\n");
        td->send_length = td->actual_senders_length;
        SEND_MESSAGE(td->target_pid, td->msg_sent, td->send_length, &Z502_REG9);
        SuccessExpected(Z502_REG9, "SEND_MESSAGE");
    }     // End of while
